async function ai_tab(link){    
  ...
  if (link === "intent"){
    ...
    mainP = `<divMain><h2>App Intents</h2><hr><div class="row"><div class="form-group col-md-12"><p>These are the intents we have for this App. When the Wit App identifies an intent in a message without entities or traits, the App will reply back with the response for this intent. If there are identified entities and/or traits, the App will respond with the response for this combination. Train the App with sample utterances to better identify each intent and combinations if any.</p><hr><div class="d-flex justify-content-center"><a class="btn btn-sm btn-success" style="color:white" onclick="ai_tab('add_intent')">Add Intent</a></div></div><hr><div class="container-fluid" style = "width:100%"><div class="table-responsive" id="tbl"><table class="table table-hover table-md" ><thead><tr><th>Intent:</th><th>Utterances:</th></tr></thead><tbody>`;
    ...
    mainP = `<divMain><h2>App Entities</h2><hr><div class="row"><div class="form-group col-md-12"><p>These are the entities we have for this App. You can add custom or built-in entities using the buttons below. If the Wit App identifies entities without an intent in a message, the App will reply with one of the responses for these entities. If there is an intent, the App will reply with one of the responses for this intent. Train the App with sample utterances to better identify each entity and combinations if any.</p><hr><div class="d-flex justify-content-center"><a class="btn btn-sm btn-success" style="color:white" onclick="ai_tab('add_entity')">Add Entity</a><a class="btn btn-sm btn-info" style="color:white; margin-left:15px;" onclick="ai_tab('built_entity')">Built-in Entities</a></div></div><hr><div class="container-fluid" style = "width:100%"><div class="table-responsive" id="tbl"><table class="table table-hover table-md" ><thead><tr><th>Entity:</th><th>Utterances:</th></tr></thead><tbody>`;
    ...
    mainP +=`<div>App Secret: <input type="password" id = "app_secret" value="${pageData.post_secret.S}"><button onclick="displayFun()">Show / Hide</button><button  style="margin-left:5px;" onclick="updateFun('secret')">Update</button><br><p id = "updateState" style="color:green; margin-top:10px"></p><hr>POST URL: <input type="url" id = "post_url" value="${pageData.post_link.S}"><button onclick="updateFun('link')">Update</button><p id = "updateState2" style="color:green; margin-top:10px"></p></div></divMain>`
  } 
    ...
    mainP +=`</div><div>App ID: <input type="text" id = "app_id" value="${pageData.app_id.S}">App Token: <input type="password" id = "app_token" value="XXX${pageData.wit_key.S.substring(5,12)}XXX"><button class="btn btn-primary btn-md" style="margin-right:15px;" id="changeButton" onclick="updateai_tab()">Change App</button><button class="btn btn-info btn-md" onclick="displayai_tab()">Show / Hide</button><p id = "updateState3" style="margin-top:10px"></p></div></divMain>`
  }
